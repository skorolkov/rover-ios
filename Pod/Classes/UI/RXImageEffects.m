...
@implementation RXImageEffects


+ (UIImage *)applyLightEffectToImage:(UIImage *)image
{
    UIColor *tintColor = [UIColor colorWithWhite:1.0 alpha:0.3];
    return [RXImageEffects applyBlurWithRadius:30 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil toImage:image];
}
...
+ (UIImage *)applyExtraLightEffectToImage:(UIImage *)image
{
    ...
    return [RXImageEffects applyBlurWithRadius:20 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil toImage:image];
}
...
+ (UIImage *)applyTintEffectWithColor:(UIColor *)tintColor toImage:(UIImage *)image
{
    ...
    return [RXImageEffects applyBlurWithRadius:10 tintColor:effectColor saturationDeltaFactor:-1.0 maskImage:nil toImage:image];
}
...
+ (UIImage *)applyBlurWithRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage toImage:(UIImage *)image
{
    ...
    if (hasBlur || hasSaturationChange) {
        ...
        if (hasBlur) {
            // A description of how to compute the box kernel width from the Gaussian             // radius (aka standard deviation) appears in the SVG spec:
